...
July 2021 - This project is a nearing release!!!
...
The concept for the remote switch has been around for awhile. After working on the matrix, it became clear it would be a huge effort to go into production. The 4 Way is a way to dip a toe in the water. There are currently (2021-03) a few units (kits). There are 2 PCB: a RF switch with SO239 and relays, and a CPU board with ESP32 (wifi capable cpu). The PCB with the relays and connectors is supplied as a kit. The CPU PCB is assembled and programmed. 

WiFi may be either Access Point mode (no router, think Field Day) or via LAN mode (home network router). The software supports user configuration of thw WiFi credentials (SSID, PW and IP).  

The weather resistant enclosure is VERY nice. There is a lip around the door and the bottom has foam to conform to the cables. The case should be considered Weather RESISTANT, not weatherproof. A lot depends on the mounting location, local weather conditions, etc. 
...
2. The 4 way is a means of easing into production and distribution. 
3. There will be a 6 Way (maybe Summer 2021) and the 2x6 Matrix a few months later. The 6 Way will have a similar (but larger) weather resistant enclosure. The Matrix will NOT.
...
5. There will be a low cost Bias T kit about in Summer 2021. Refer to the mailing list for details
...
The ESP32 CPU comes with either a builtin WiFi antenna or a version with a connector for an external antenna which the user can install by drilling a hole for an SMA connector and adding a rubber duck whip. These photos will be updated as needed.
...
!https://github.com/xcrmaker/4-Way-Remote-RF-Switch/blob/main/XCR-4w-001-Select_Antenna.png
